...
\textbf{Structured Label Co-occurrence.} Despite this imbalance, the relationships between labels are not random but exhibit meaningful patterns. Co-occurrence analysis reveals that while most pairs of narratives rarely appear together, certain thematically-linked pairs exhibit strong associations. For instance, the most frequent co-occurring pair is present in approximately 8\% of all documents. More advanced analysis reveals directional dependencies, where the presence of one label strongly predicts another (i.e., \(P(B\mid A) \neq P(A\mid B)\)). Metrics such as Pointwise Mutual Information (PMI) highlight pairs that co-occur more often than expected by chance, while Jaccard similarity identifies pairs with substantial overlap. When viewed as an undirected graph, the label co-occurrence matrix is sparse, indicating that dependency modeling can focus on a smaller, more meaningful subgraph of label relationships rather than a fully connected structure.
