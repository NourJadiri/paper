...
\noindent\textbf{Overview.} A critical first step in our methodology, applied universally before any model training or inference, was a comprehensive data pre-processing pipeline. The raw documents, sourced from online news articles, contained non-content artifacts typical of web scrapes, such as residual HTML tags, navigation links, and boilerplate phrases (e.g., ``Share this article''). This noise both dilutes the core narrative content and consumes a valuable portion of the model's input context window (XLM-RoBERTa uses up to 512 tokens).

\medskip
\noindent\textbf{Cleaning operations.} The pipeline aimed to isolate the main body of each article and produce clean, normalized prose. Typical operations included removing HTML and scripting remnants, filtering common boilerplate phrases, and normalizing whitespace to ensure consistent formatting. These steps raise the signal-to-noise ratio of the inputs and help models focus their representational capacity on semantically relevant content.

\medskip
\noindent\textbf{Implementation detail.} Rather than constructing a suite of brittle regular expressions or deterministic heuristics, we found a lightweight language model to be a highly effective and low-effort option for this task. The LLM was used to identify and excise non-content artifacts while preserving the surrounding text and authorial tone; in practice, this approach removed navigation elements and calls-to-action embedded inside paragraphs while keeping the original prose intact.

\medskip
\noindent\textbf{Intended effect.} By ensuring higher-quality input text, the pre-processing pipeline improved downstream classification accuracy across our architectures by preventing models from learning spurious correlations tied to irrelevant boilerplate.
...
\medskip
\noindent\textbf{Methodology.} We leveraged the state-of-the-art translation capabilities of a large language model (Gemini~1.5~Flash) to augment our training data. Our process was as follows:
